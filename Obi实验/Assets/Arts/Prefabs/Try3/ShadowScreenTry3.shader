Shader "Custom/ShadowScreenTry3"
{
    Properties
    {
        _MainTex ("Curtain Texture", 2D) = "white" {}
        _NormalMap ("Curtain Normal", 2D) = "bump" {}
        _ShadowTex ("Shadow RT", 2D) = "black" {}
        _PuppetTex ("Puppet RT", 2D) = "white" {}
        _PuppetStrength ("Puppet Strength", Range(0, 1)) = 1
        _ShadowBlend ("Shadow Blend", Range(0, 1)) = 1
        _ShadowStrength ("Shadow Strength", Float) = 1
        _ShadowTint ("Shadow Tint", Color) = (0.2, 0.2, 0.2, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_PuppetTex); SAMPLER(sampler_PuppetTex);
            ...
            float SampleShadowBlur(float2 uv, out float shadowBlend)
            {
                float4 center = SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv);
                shadowBlend = center.b;
                float blurPixels = center.g * _BlurScale;
                if (blurPixels <= 0.001)
                {
                    return center.r;
                }
                ...
                float2 blur = texel * blurPixels;
                float sum = 0.0;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(-1, -1)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(0, -1)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(1, -1)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(-1, 0)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(1, 0)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(-1, 1)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(0, 1)).r;
                sum += SAMPLE_TEXTURE2D(_ShadowTex, sampler_ShadowTex, uv + blur * float2(1, 1)).r;
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                float shadowValue = SampleShadowBlur(IN.uv, shadowBlend);
                shadowValue = ApplySaturation(shadowValue.xxx, _Saturation).r;
                ...
                float shadowStrength = saturate(shadowValue * _ShadowStrength);
                float3 shadowMul = lerp(1.0.xxx, _ShadowTint.rgb, shadowStrength);

                float3 puppetTex = SAMPLE_TEXTURE2D(_PuppetTex, sampler_PuppetTex, IN.uv).rgb;
                float3 puppetMul = lerp(1.0.xxx, puppetTex, _PuppetStrength);
                float perPixelBlend = saturate(shadowBlend);
                float usePuppet = step(0.001, _PuppetStrength);
                float blend = lerp(1.0, perPixelBlend * _ShadowBlend, usePuppet);
                float3 finalMul = lerp(puppetMul, shadowMul, saturate(blend));

                float3 multiplied = baseLit * finalMul;
